...
`nypl-global-search-production` and `nypl-global-search-qa` are belonged to `nypl-digital-dev` on AWS. and `nypl-global-search-development` is belonged to `nypl-sandbox`. For checking the activities either of the AWS accounts, log in
...
Sometimes, we need to test a feature branch at one of the environments, so we need to deploy the branch. For doing that, change the value of `branch` configuration from `master`, `qa`, or `development` to the name of the feature branch. Save, commit, and push it just as the regular deployments.
